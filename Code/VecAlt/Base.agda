...
find {l = suc l-1} (x ∷ xs) e =
                  if x ≤? e
                  then[ ≤-refl ] if e ≤? x
                       then[ ≤-trans (s≤s z≤n) [l-1]-suc ] return (just $ Fin.zero)
                       else[ [l-1]-suc ] (find xs e <&> λ res -> Data.Maybe.map (Fin.suc) res)
                  else[ ≤-step $ [l-1]-suc ] (find xs e <&> λ res -> Data.Maybe.map (Fin.suc) res)
    where
        [l-1]-suc = ≤-reflexive $ sym $ +-suc l-1 (l-1 + 0)
...
removeElem {l = suc l-1} (x ∷ xs) e =
                  if x ≤? e
                  then[ ≤-refl ] if e ≤? x
                       then[ ≤-trans (s≤s z≤n) (≤-reflexive (sym $ +-suc l-1 (l-1 + 0))) ] return $ decrement xs
                       else[ ≤-reflexive $ sym $ +-suc l-1 (l-1 + 0) ] (x ∷'_ <$> removeElem xs e)
                  else[ ≤-step (≤-reflexive (sym $ +-suc l-1 (l-1 + 0))) ] (x ∷'_ <$> removeElem xs e)
    where
